...
@WebServlet("/controller/GuardianDelete.do")
public class GuardianDelete extends HttpServlet{
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(result == 1) {
			...
			FileUtil.deleteFile(req, "data", fileName);
			
			req.getRequestDispatcher("guardian_list.do").forward(req, resp);		
		}
		else {
			req.setAttribute("delete", "삭제 실패");
			req.getRequestDispatcher("guardian_view").forward(req, resp);
		}
