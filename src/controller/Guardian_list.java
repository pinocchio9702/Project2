...
@WebServlet("/controller/guardian_list.do")
public class Guardian_list extends HttpServlet{
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//테이블의 전체 레코드 수를 카운트
		int totalRecordCount = dao.getTotalRecordCountGuardian(param);
		...
		String pagingImg = PagingUtil.paginBS4(totalRecordCount, pageSize, blockPage, nowPage,
				"guardian_list.do?" + addQueryString);
		param.put("pagingImg", pagingImg);
		
		List<Multi_boardDTO> listsGuardian = dao.selectListPageGuardian(param); //페이지 처리 o
		
		//DB연결을 헤제하는 것이 아니라 커넥션풀에 개체를 반납한다.
		dao.close();
		
		//데이터를 request영역에 저장한다.
		req.setAttribute("listsGuardian", listsGuardian);
